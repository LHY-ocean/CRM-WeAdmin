...
public class UserRealm extends AuthorizingRealm
{
    ...
    UsernamePasswordToken token =  (UsernamePasswordToken) arg0;
        String username = token.getUsername();
         ...
         //假设找到的用户如下
         Operator operator =  oservice.selectOperator(new Operator(token.getUsername(),new String(token.getPassword())));
        if (null == operator)
        {
            throw new  AccountException("username is not exist");
        }
        SecurityUtils.getSubject().getSession().setAttribute("user", operator);
